...
import { AppNames, getDownloadConfig } from './download';
...
export const getTargetInfo = (options: CallAppOptions) => {
  ...
  if (!targetApp) {
    logError(
      `(targetApp || appName) '${targetApp}' is Invalid， please check! \n`
    );
    return;
  }
  ...
  return { downloadConfig, flag, name, schemePrefix, universalPath };
};

const isZZ = (targetApp: string): boolean =>
  /^(zhuanzhuan|zz)$/i.test(targetApp);

const isZZSeller = (targetApp: string): boolean =>
  /^zzSeller$/i.test(targetApp);

const isZZHunter = (targetApp: string): boolean =>
  /^zzHunter$/i.test(targetApp);
...
const isZZSellerPrefixPath = (path: string): boolean =>
  /^zhuanzhuanseller:/i.test(path);

const isZZHunterPrefixPath = (path: string): boolean =>
  /^zzhunter:/i.test(path);
