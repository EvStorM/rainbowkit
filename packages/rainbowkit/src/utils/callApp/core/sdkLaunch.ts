...
import { openZZInnerApp, openZZInWX } from '../libs/sdk/index';
...
export const sdkLaunch = async (instance: CallAppInstance) => {
  ...
  const { callError = () => {}, callFailed = () => {} } = options;
  if (!targetInfo)
    return logError(`please check options.targetApp is legal, ${targetInfo}`);
  try {
    if (isWechat) {
      // wx-js-sdk
