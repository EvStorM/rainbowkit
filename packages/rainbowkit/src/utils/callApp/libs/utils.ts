...
export const loadJS = (url: string, cb: () => void) => {
  ...
  const js = (window as any).document.createElement('script');
...
export function copy(text: string, options?: Record<string, any>): boolean {
  ...
  try {
    ...
    const yPosition =
      (window as any).pageYOffset || document.documentElement.scrollTop;
...
// 默认关闭 调试信息
// (window as any).__callAppDev__ = false;
// 默认打开 异常信息
// (window as any).__callAppError__ = true;
//
